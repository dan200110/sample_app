  ...
  module V1
    module Ad
      class InventoriesController < Base
        ...
        def index
          @inventories = Inventory.search_by_name(params["search_name"]).search_by_branch(params["branch_id"])
        ...
        def get_expired
          ...
          @expired_inventories = Inventory.where(batch_inventory_id: batch_expired).search_by_branch(params["branch_id"])
        ...
        def get_out_of_stock
          ...
          @out_of_stock_inventories = Inventory.get_out_of_stock(quantity_left).search_by_branch(params["branch_id"])
        ...
        def send_request_mail_to_supplier
          quantity_left = params[:quantity_left].present? ? params[:quantity_left].to_i : 0
          @list_inventories_mail = Inventory.get_out_of_stock(quantity_left)
        ...
        def inventory_params
          params.permit(:name, :price, :inventory_type, :quantity, :category_id, :batch_inventory_id,
                        :supplier_id, :image, :main_ingredient, :producer, :inventory_code, :branch_id)
        end
